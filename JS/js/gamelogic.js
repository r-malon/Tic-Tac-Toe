...
var virtualGrid, gameEnded = false;
...
function checkVictory() {
	...
	return checkAntiDiagonal() || checkDiagonal();
}
...
function checkDiagonal() {
	...
	for (let i = 0; i < N; i++) {
		if (isNaN(virtualGrid[i][i])) return false;
		sum += virtualGrid[i][i];
	}
	return sum == N || sum == 0;
}
...
function checkAntiDiagonal() {
	...
	for (let i = 0; i < N; i++) {
		if (isNaN(virtualGrid[i][(N-1) - i])) return false;
...
function checkColumn(column) {
	...
	for (let i = 0; i < N; i++) {
		if (isNaN(virtualGrid[i][column])) return false;
